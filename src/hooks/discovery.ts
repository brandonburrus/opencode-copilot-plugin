 ...
 *
 * Scans two locations:
 * - `<projectRoot>/.github/hooks/*.json` — project-local hooks
 * - `~/.copilot/hooks/*.json` — user-global hooks
 *
 * For each hook type, project hook arrays are prepended to global hook arrays so that
 * project hooks always run before global hooks.
 *
...
export async function discoverHookRegistry(projectRoot: string): Promise<HookRegistry> {
  ...
  return mergeRegistries(projectHooks, globalHooks)
}
